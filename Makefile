...
## Local test helpers
produce_test_event:
	EVENT_BUS_PRODUCER='edx_event_bus_redis.create_producer' EVENT_BUS_REDIS_CONNECTION_URL='redis://:password@localhost:6379/' python manage.py produce_event --signal openedx_events.content_authoring.signals.XBLOCK_DELETED --topic dev-xblock-deleted --key-field None --data '{"xblock_info": {"usage_key": "block-v1:edx+DemoX+Demo_course+type@video+block@UaEBjyMjcLW65gaTXggB93WmvoxGAJa0JeHRrDThk", "block_type": "video"}}'

consume_test_event:
	EVENT_BUS_PRODUCER='edx_event_bus_redis.create_producer' EVENT_BUS_REDIS_CONNECTION_URL='redis://:password@localhost:6379/' python manage.py consume_events --signal org.openedx.content_authoring.xblock.deleted.v1 --topic dev-xblock-deleted --group_id test_group --consumer_name test_group.c1

multiple_consumer_test_event:
	EVENT_BUS_PRODUCER='edx_event_bus_redis.create_producer' EVENT_BUS_REDIS_CONNECTION_URL='redis://:password@localhost:6379/' python manage.py consume_events --signal org.openedx.content_authoring.xblock.deleted.v1 --topic dev-xblock-deleted --group_id test_group --consumer_name test_group.c1 &
	EVENT_BUS_PRODUCER='edx_event_bus_redis.create_producer' EVENT_BUS_REDIS_CONNECTION_URL='redis://:password@localhost:6379/' python manage.py consume_events --signal org.openedx.content_authoring.xblock.deleted.v1 --topic dev-xblock-deleted --group_id test_group --consumer_name test_group.c2 &
